...
sigset_t vide, plein; // crée signaux vides 
...
//insere un objet dans les tableaux selon la variable index.
...
{		/* on doit "trier" chronologiquement les jalons  , on va donc traiter ceux*/
  ...
  getitimer( ITIMER_REAL, &curr_timer);
  	/*On compare les différents temps des événements pour savoir si on utilise l'indice i ou curr_index*/
		...
		/*si le timer que nous voulons amorcer est plus long que celui qui est en cours il faut changer les valeurs et prendre celles en cours */
